...
| The budget meter is not fabricated                      | **Real gas units and a real fee in wei** — `gasUsed × gasPriceWei + l1Fee`, verified equal to the wei that left the payer on chain. **USD is notional** at a frozen price (G-04, G-18). **KeeperHub sponsors on 84532, so no wallet balance moves** — the meter is a policy limit, not a draining account. Payment leg unused until CVY-017 (G-17) | `services/worker/src/budget.ts` · `services/worker/test/budget.meter.test.ts`                                        | CVY-007           | ENFORCED |
