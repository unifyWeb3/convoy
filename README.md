...
| The budget meter is not fabricated                      | **Real gas units and a real fee in wei** — `gasUsed × gasPriceWei + l1Fee`, verified equal to the wei that left the payer on chain. **USD is notional** at a frozen price (G-04, G-18). **Sponsorship is partial** — most writes are sponsored, but a 12-item burst produced one the org wallet paid for itself (DEC-006), so the meter is mostly a policy limit and occasionally a real spend. Payment leg unused until CVY-017 (G-17) | `services/worker/src/budget.ts` · `services/worker/test/budget.meter.test.ts`                                        | CVY-007           | ENFORCED |
