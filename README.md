...
## Dispatch width does not change throughput

Convoy dispatches ready items concurrently at `EXECUTE_FANOUT`. It does not order them — KeeperHub's
nonce manager does. If that were not true, widening dispatch would change how fast a batch lands.

The same 12-item batch, run twice (DEC-007):
