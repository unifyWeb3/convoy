...
Build output is excluded (`out/`, `dist/`, `.next/`): CI checks out a clean tree, but you are running these
...
grep -rn "app\.keeperhub\.com" apps packages services scripts \
  ...
  --exclude-dir=out --exclude-dir=dist --exclude-dir=.next --exclude-dir=node_modules | grep -v "^packages/kh-client/"
...
grep -rln "PRIVATE_KEY" apps packages services scripts \
  --exclude-dir=out --exclude-dir=dist --exclude-dir=.next --exclude-dir=node_modules | grep -v "^packages/contracts/script/"

# no staged failures in non-test code
grep -rnE "mockRevert|fakeTxHash|throw new Error\(\"fake" apps packages services scripts \
  --include=*.ts --include=*.tsx \
  --exclude-dir=out --exclude-dir=dist --exclude-dir=.next --exclude-dir=node_modules | grep -vE "(test|spec|__tests__|fixtures)"
```
