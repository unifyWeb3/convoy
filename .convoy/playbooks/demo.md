...
| b   | Base RPC issues           | Reads use the dedicated pinned RPC (8453 unconditionally). Hot-swap `BASE_RPC_URL_FALLBACK`. Manifest reconciliation is cached from the run, so it renders without a live RPC.                                                                                                                                                            |
