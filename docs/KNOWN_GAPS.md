...
| G-33 | **All four BullMQ phase handlers are still skeletons**, three milestones after the behaviour they name shipped. `plan`, `critique`, `execute` and `seal` log a line and return `done`; the live driver is `runBatch`, which calls the orchestrator directly                                                                          | the queue path — retries, stalled-job re-pick, concurrency, SIGTERM drain — is exercised by tests but drives no real run, so crash-resume has nothing to resume                                                      | **Recorded, not fixed at CVY-011.** Filling in `critique` alone would put a real critique step in front of an execute step that does nothing. The handlers connect to the orchestrator at CVY-015, where crash-resume needs them; `runBatch` stays the driver until then and is honest about it | OPEN      |
...
| G-43 | The GATE 2 card names `POST /api/runs` and `tests/fixtures/batch-12item.json`, but the route still returns 501 and the named fixture does not exist                                                                                                                                                                                  | the documented operator entrypoint cannot start the full run, and the gate cannot prove the queue-backed browser submission path                                                                                     | The gate used a dedicated direct `runBatch` harness against the same orchestrator and real KeeperHub path. Connect the API/queue path at CVY-015 with G-33 rather than inventing a second execution path                                                                                        | OPEN      |
...
### G-43 — the documented GATE 2 start path is absent **(OPEN)**

`POST /api/runs` returns 501 and `tests/fixtures/batch-12item.json` is absent. The gate therefore
created the validated twelve items with `services/worker/scripts/gate2.mjs` and called the existing
`runBatch` driver directly. This exercised the real plan phase, simulations, deferral gate,
KeeperHub writes, sealing and manifest export, but it did not prove an API-to-queue submission path.

This is the same unfinished boundary as G-33: the BullMQ phase handlers connect to the orchestrator
at CVY-015. GATE 2 records the fallback rather than adding an alternate run-creation API.
