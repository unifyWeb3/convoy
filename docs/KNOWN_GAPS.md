...
| G-15 | Compiling `Deploy.s.sol` emits `packages/contracts/out/Deploy.s.sol/Deploy.json`, and Next emits `apps/web/.next` bundles; generated output contains guarded vocabulary and tripped local grep-guards                                                                                                                                | CI checks out a clean tree, but reviewers run guards after builds and a false red result trains them to ignore a real invariant                                                                                      | Guards 1, 3 and 4 exclude `out/`, `dist/`, `.next/` and `node_modules/`; the same exclusions are mirrored in `.convoy/checklists/review.md`. Guard semantics remain unchanged for source files                                                                                                                                                                                                                                                                              | MITIGATED |
...
| G-40 | GATE 2 at execution fanout 4 produced two real KeeperHub wallet `InvalidNonce()` reverts; the same 12-item batch completed at fanout 1. Convoy did not set a nonce                                                                                                                                                                   | wider full-demo fanout is not currently stable even though KeeperHub is expected to serialize submissions                                                                                                            | **Mitigated at CVY-016 pre-live correction:** production now defaults to `fanout=1`; wider fanout requires an explicit measurement/rehearsal override. The failed and serial runs remain separate real evidence                                                                                                                                                                                                                                                             | MITIGATED |
| G-41 | The configured Base Sepolia RPC free tier accepts `eth_getLogs` over at most ten blocks; the manifest exporter requested the full open-to-seal range                                                                                                                                                                                 | the registry source was unavailable for the 12-item export, so all rows were honestly amber even though direct post-run storage reads agreed                                                                         | **Mitigated at CVY-016 pre-live correction:** open/commit/seal log reads are chunked into at most ten-block ranges, merged deterministically and deduplicated. Historical amber exports remain historical evidence                                                                                                                                                                                                                                                          | MITIGATED |
| G-42 | The raw viem/provider error stored in the manifest included the complete configured RPC URL, whose path contained the provider credential                                                                                                                                                                                            | raw manifest JSON, registry error cells, copied/downloaded JSON and unmasked screenshots could disclose the RPC credential                                                                                           | **Mitigated at CVY-016 pre-live correction:** provider errors are sanitized before return/export, and registry reads fall back to a separately chain-pinned 84532 provider. Previously exposed credentials still require rotation outside this repository                                                                                                                                                                                                                   | MITIGATED |
...
| G-44 | CVY-016's illustrative planner-ablation result assumed dependency-violating target calls would broadcast and consume gas, but the production lifecycle retains KeeperHub `simulate:true` after Planner ablation. On the accepted fresh run, out-of-order items were vetoed before broadcast.                                         | The measured Planner ablation can show reduced executable work without manufacturing wasted-gas events.                                                                                                              | The normal simulate/Critic gate remains enabled for `--ablate-planner`; README and the milestone report the measured result (1 submitted/landed item, 0 wasted-gas events) rather than the illustrative expectation.                                                                                                                                                                                                                                                        | ACCEPTED  |
| G-45 | CVY-016 requires equivalent fresh contract state for three independent runs, while `MockRewardDistributor.root` is one-shot and enabled markets are persistent.                                                                                                                                                                      | Reusing a mutated address would contaminate the comparison.                                                                                                                                                          | Three fresh distributors were deployed and used exactly once each: baseline `0xCD3c…f567`, Planner ablation `0x5c52…E108`, and Critic ablation `0x350e…Af1B`. The accepted artifacts preserve each address and run identity.                                                                                                                                                                                                                                                | CLOSED    |
| G-46 | The Planner/Critic source modules lived under `apps/web/lib`, while the production worker is compiled with `rootDir: services/worker/src`; direct imports failed TypeScript's root boundary.                                                                                                                                         | the queue lifecycle could execute only the deterministic/simulator fallback even when LLM configuration existed                                                                                                      | **Mitigated at CVY-016.** The existing implementation is shared through `@convoy/ai`; web compatibility exports and the worker composition adapter use the same Planner/Critic code.                                                                                                                                                                                                                                                                                        | MITIGATED |
| G-47 | The previously configured external Responses providers cannot currently authenticate under the zero-spend constraint.                                                                                                                                                                                                                | CVY-016 could not accept a baseline if Planner or Critic degraded to deterministic/simulator-only fallback.                                                                                                          | **Mitigated at CVY-016:** the stateless Testnet Bradbury Intelligent Contract and accountless `simulateWriteContract` provider sit behind the unchanged `LlmCaller` boundary; the real-role preflight passed and the accepted baseline records a genuine GenLayer Planner result.                                                                                                                                                                                           | MITIGATED |
| G-48 | Critic ablation's two invalid submissions were rejected by KeeperHub before target broadcast, so no target receipt exists for them.                                                                                                                                                                                                  | The illustrative target-revert/wasted-gas outcome was not observed in this run.                                                                                                                                      | Accepted as measured evidence. They are documented as two KeeperHub pre-broadcast rejections, **not** onchain reverts or wasted gas; no rerun is warranted.                                                                                                                                                                                                                                                                                                                 | ACCEPTED  |
...
### G-40 — wider fanout exposed a real KeeperHub nonce failure **(MITIGATED)**
...
The canonical production default is now one. Wider fanout remains available only as an explicit
measurement/rehearsal override; the historical fanout-4/12 timings are not current stability evidence.

### G-41 — RPC log-range limit made the full manifest amber **(MITIGATED)**
...
result but are not substituted into the historical manifest. CVY-016 now chunks every event scan
into ranges of at most ten blocks, merges them deterministically and removes identical logs.

### G-42 — raw provider errors disclosed the RPC credential **(MITIGATED)**
...
The GATE 2 browser spec masks the historical registry-error column. The product surface now sanitizes
provider errors before storage/export/return and validates both primary and fallback providers as
Base Sepolia 84532. Any credential exposed before this correction still requires rotation outside
the repository.
...
### CVY-016 residual verification friction

PostgreSQL and Redis were unavailable on the repository's default local ports during the residual
accounting correction. Service-backed verification therefore used disposable processes and data
directories under `/tmp`, with PostgreSQL on 55433 and Redis on 6380. Only the checked-in migrations
and deterministic seed were applied; no user database was touched. Serializable contention tests
print Prisma's expected write-conflict diagnostics while the retry path succeeds.

The root `pnpm format:check` glob also scans the user-owned, untracked `design/` handoff and reports
its generated files. Those files are outside CVY-016 scope and were left unchanged. Running the same
Prettier check over repo-controlled files excluding `design/` and `designprompt/` passes. This does
not affect clean-checkout CI, where those untracked directories do not exist.
