...
| G-20 | `revertReason` is not the documented `Error(...)` string — the live API returns an ethers v6 diagnostic blob (`Simulation reverted: missing revert data (action="call", …, code=CALL_EXCEPTION, version=6.16.0)`)   | the Critic keys on `revertReason`; a diagnostic blob is far weaker evidence than a decoded custom error, and CVY-011's veto quality depends on it                               | Client passes the blob through verbatim rather than parsing it. Decode quality against a **custom-error** contract is unverified — WETH9's `require` carries no revert data. Re-measure against `MockRewardDistributor` at CVY-003 | OPEN      |
...
### G-20 — `revertReason` is a diagnostic blob, not `Error(...)`

`CLAUDE.md` documents `400 {wouldRevert:true, revertReason:"Error(...)"}`. Measured on 2026-08-03 the
field instead carries an ethers v6 diagnostic string:

```
Simulation reverted: missing revert data (action="call", data=null, reason=null,
transaction={ "data": "0x2e1a7d4d…", "from": "0x65F5…", "to": "0x4200…" },
invocation=null, revert=null, code=CALL_EXCEPTION, version=6.16.0)
```

**This matters for CVY-011.** The Critic's veto evidence is `revertReason`; "missing revert data" is
a much weaker artifact to show a judge than a decoded `RootNotSet()`.

**Not yet attributable.** The probe targeted WETH9, whose `withdraw` uses a bare `require` and so
emits _no revert data at all_ — `data=null` may be the contract's fault rather than the API's.
`MockRewardDistributor` reverts with **custom errors**, which carry 4-byte selectors and should decode
far better. **Re-measure at CVY-003 once it is deployed** before concluding anything about the API.
The client passes the string through verbatim either way; nothing is parsed out of it.
