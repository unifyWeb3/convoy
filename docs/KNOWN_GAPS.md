...
| G-32 | `packages/db/test/schema.migrate.test.ts` "the five frozen tables exist" **failed twice under `pnpm -r test` and passed on every rerun and in isolation**. Not root-caused                                                                                                                                                           | a flaky gate is a gate nobody trusts; worse, it trains a reader to rerun until green, which is how a real failure gets waved through                                                                                 | **Recorded, not fixed.** Observed 2026-08-04 during CVY-010. `pnpm -r test` runs packages in parallel and both `@convoy/db` and `@convoy/worker` now drive the same Postgres, so a race or connection-pool exhaustion is the leading hypothesis — untested. Investigate before GATE 2 rather than at it | OPEN      |
...
| G-35 | `executeItem` records a COMMIT attempt and advances `SIMULATED → COMMITTED` without checking that `commitAction` finished `completed` with a transaction hash; it then submits the item write anyway                                                                                                                                 | an item write can land without its registry commitment, violating the execution ordering claim; CVY-012 will show amber but cannot prevent the spend                                                                 | **Recorded at CVY-012, not fixed outside the milestone.** Add the same completed+hash guard used for LANDED before the item write, and pin it with a failed-commit test before GATE 2                                                                                                                   | OPEN      |
