...
| G-26 | BullMQ re-picks a stalled job after ~30s (`stalledInterval`). The re-picked job re-runs the handler, which for an EXECUTE job means re-issuing a write that may already be in flight or already landed                                                                                                                               | a double submission on chain — real gas, duplicate state — from a crash-resume path that believed it was safe                                                                                                        | **MITIGATED at CVY-015.** Attempts are prepared before submission; the execution ID is persisted before polling. A re-picked job polls that ID, or reissues only the same persisted phase-folded key when no ID was durably recorded. Deterministic kill/restart proof records one broadcast/hash/event | MITIGATED |
...
| G-33 | **All four BullMQ phase handlers were skeletons**; the live driver was `runBatch`, which called the orchestrator directly                                                                                                                                                                                                            | the queue path — retries, stalled-job re-pick, concurrency, SIGTERM drain — exercised no real run, so crash-resume had nothing to resume                                                                             | **MITIGATED at CVY-015.** The handlers delegate to the single resumable `runBatch`/orchestrator lifecycle. POST `/api/runs` enqueues an existing persisted run; no alternate execution path was added                                                                                           | MITIGATED |
...
| G-43 | The GATE 2 card named `POST /api/runs`, but the route returned 501; the named fixture still does not exist                                                                                                                                                                                                                          | the documented operator entrypoint could not start the queue-backed run path                                                                                                                                          | **PARTIALLY MITIGATED at CVY-015.** POST `/api/runs` now validates an existing run ID and enqueues the one production lifecycle. It intentionally does not invent a run-creation surface or fixture. Live browser submission remains future demo evidence                                        | MITIGATED |
...
**Mitigated at CVY-015:** the attempt is prepared before submission and KeeperHub's `executionId`
is persisted before polling. A resumed/re-picked job polls that ID first. If the process died before
the ID was durably recorded, it reissues only the same persisted attempt/key. Completed-without-hash
never becomes `LANDED`; coded transient failures advance only under the retry cap; config reverts
are terminal. The deterministic kill/restart transport proves one target broadcast, one hash and one
terminal event.
...
### G-43 — the documented GATE 2 start path was absent **(MITIGATED)**

CVY-015 connects the minimum production entrypoint: `POST /api/runs` validates an already-persisted
run ID and enqueues it, and every BullMQ phase handler delegates to the single resumable
`runBatch`/orchestrator path. It deliberately does not invent a second run-creation surface or the
missing fixture. A future demo may add live browser evidence, but the 501/skeleton boundary is gone.
...
The live KeeperHub kill/restart acceptance could not be performed in this worker process because
the required KeeperHub/Base Sepolia runtime configuration was unavailable. No live run ID,
execution ID, transaction hash or duplicate-hash SQL result is fabricated; the deterministic
transport proof is reported separately.
