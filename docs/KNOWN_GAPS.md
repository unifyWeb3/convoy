...
| G-18 | **Budget meter is notional, and sponsorship is PARTIAL.** Gas units and the fee in wei are real; the USD price is frozen; KeeperHub sponsors **most but not all** writes — under a 12-item burst one write came from the org wallet, which paid its own fee | budget figures are notional; the wallet DOES sometimes spend, so funding it is load-bearing                                                                                     | Disclose: "real gas units, real fee in wei, notional USD at a frozen price, mostly sponsored." Measured in DEC-004, **amended by DEC-006**                                                                                                                         | ACCEPTED  |
...
| G-24 | A KeeperHub-executed transaction's receipt `to` is the sponsoring relay (`0x5af5194b…7f07d`), not the target contract, and `from` is a relay EOA, not the org wallet                                                                                        | reconciling the manifest's onchain leg by `to == CONVOY_REGISTRY_ADDR` or `from == org wallet` would report a false mismatch on every single item                               | Match on the **emitting contract of the log** and on the `RunOpened`/`ActionCommitted` indexed `operator` topic, both of which are the real values. Verified against the CVY-003 first transaction                                                                 | OPEN      |
...
| G-26 | BullMQ re-picks a stalled job after ~30s (`stalledInterval`). The re-picked job re-runs the handler, which for an EXECUTE job means re-issuing a write that may already be in flight or already landed                                                      | a double submission on chain — real gas, duplicate state — from a crash-resume path that believed it was safe                                                                   | **Idempotency-Key carries the attempt number, and the attempt is fixed in `job.data` at enqueue (DEC-003).** A re-picked job produces a byte-identical key, so KeeperHub's per-org 24h window collapses it to one execution. Reconcile-before-act lands at CVY-015 | MITIGATED |
...
| G-28 | KeeperHub's `gasUsedWei` carries gas **UNITS**, not wei; and its status response omits the OP-stack **L1 data fee** entirely                                                                                                                                | feeding units into the frozen formula understates cost ~6.2 million×; dropping the L1 fee understates it a further ~2.9%                                                        | Client exposes `gasUsedUnits` + `gasPriceWei` so the wrong division is unreachable; `composeGasFeeWei` adds `l1Fee` read from the receipt via `BASE_RPC_URL`, and flags `l1FeeIncluded:false` when it could not                                                    | MITIGATED |
...
| G-30 | **Demo-narration risk.** §15 attributes nonce serialization to "one wallet, one sequential nonce" — the org Turnkey wallet. Measured, the serializing nonce belongs to a KeeperHub **relay EOA**; `from` on Basescan is not the org wallet                  | a judge opening any transaction sees a `from` the narration does not mention. Serialization itself is real and backup path d survives; the **attribution** is the overclaim     | **Reported, not fixed.** DEC-006 records the measurement. The narration is the operator's to adjust at CVY-019                                                                                                                                                     | OPEN      |
...
**Still owed at CVY-015:** reconcile-before-act. Idempotency prevents a duplicate _submission_; it
does not by itself tell Convoy what happened to the original. A resumed run must query
`GET /api/execute/{id}/status` for the recorded `executionId` before deciding anything, and must
never re-broadcast without the original key. `stalledInterval` and `maxStalledCount` are set
explicitly in `services/worker/src/config.ts` rather than left at defaults, so the timing is a
decision the crash-resume test can reason about.
...
### G-30 — the narration attributes the nonce to the wrong wallet **(OPEN)**
...
**Reported, not fixed.** The narration belongs to CVY-019 and to the operator.
