...
| G-25 | The KeeperHub Claude Code plugin is **not installed**. Installing it requires `/plugin …` REPL commands typed by a human and a browser OAuth consent; an agent session can do neither                               | criterion 2 names MCP, so the plugin is a judged surface, and its authenticated tool list is unmeasured                                                                         | **Does not block anything.** Convoy reaches the same `/mcp` endpoint with `kh_` Bearer auth today, and the plugin declares no new endpoint. Manifest inventory recorded in `.convoy/mcp/README.md`; operator runs four commands to finish it | OPEN      |
...
### G-25 — the KeeperHub Claude Code plugin is not installed **(OPEN)**
...
**To close:** an operator runs the four commands in `.convoy/mcp/README.md`, then records the
authenticated `/keeperhub:status` tool list. **The README surfaces list must not name the plugin
until it has genuinely been used** — listing an uninstalled plugin as a surface would be a claim
without an artifact, which is the one thing the honesty table exists to prevent.
